...
[Shared]
public sealed class ConditionalAssignmentCodeFix : CodeFixProvider
{
    /// <inheritdoc />
    public override ImmutableArray<string> FixableDiagnosticIds => [RuleIdentifiers.SimplifyConditionalAssignment];
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        context.RegisterCodeFix(
            CodeAction.Create(
                "Simplify conditional assignment to ternary",
                ct => FixAsync(context.Document, ifStatement, ct),
                equivalenceKey: nameof(ConditionalAssignmentCodeFix)
            ),
            diagnostic
        );
    }
    ...
    private static async Task<Document> FixAsync(Document document, IfStatementSyntax ifStatement, CancellationToken ct)
    {
        ...
        var ifAssignment = GetSingleAssignment(ifStatement.Statement);
        var elseAssignment = GetSingleAssignment(ifStatement.Else!.Statement);

        var target = ifAssignment.Left.WithoutTrivia().ToFullString();
        var condition = ifStatement.Condition.WithoutTrivia().ToFullString();
        var trueValue = ifAssignment.Right.WithoutTrivia().ToFullString();
        var falseValue = elseAssignment.Right.WithoutTrivia().ToFullString();

        var replacementText = $"{target} = {condition} ? {trueValue} : {falseValue};";
