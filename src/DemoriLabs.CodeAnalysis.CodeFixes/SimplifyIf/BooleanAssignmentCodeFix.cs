...
[Shared]
public sealed class BooleanAssignmentCodeFix : CodeFixProvider
{
    /// <inheritdoc />
    public override ImmutableArray<string> FixableDiagnosticIds => [RuleIdentifiers.SimplifyBooleanAssignment];
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        context.RegisterCodeFix(
            CodeAction.Create(
                ...
                equivalenceKey: nameof(BooleanAssignmentCodeFix)
            ),
            diagnostic
        );
    }
    ...
    private static async Task<Document> FixAsync(Document document, IfStatementSyntax ifStatement, CancellationToken ct)
    {
        ...
        var ifValue = GetBooleanLiteralValue(ifAssignment.Right);
        var target = ifAssignment.Left.WithoutTrivia().ToFullString();
        var condition = ifStatement.Condition.WithoutTrivia().ToFullString();
        var replacementText = ifValue ? $"{target} = {condition};" : $"{target} = {condition} is false;";
